...
function createClient(config, app) {
  ...
  if (config.cluster === true) {
    ...
    assert((config.host && config.port && config.password !== undefined && config.db !== undefined) || config.path || typeof config === 'string',
      `[egg-redis] 'host: ${config.host}', 'port: ${config.port}', 'password: ${config.password}', 'db: ${config.db}' or 'path:${config.path}' or 'connection string:${config}' are required on config`);
    if (config.host) {
